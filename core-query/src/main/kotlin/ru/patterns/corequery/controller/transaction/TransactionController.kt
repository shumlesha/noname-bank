...
import ru.patterns.corequery.controller.transaction.serialization.AccountIdRaw
import ru.patterns.corequery.controller.transaction.serialization.Factory
...
class TransactionController(
    ...
    @PostMapping("/account")
    fun getAllAccountTransactions(@RequestBody accountIdRaw: AccountIdRaw): Mono<TransactionResponse> =
        Mono.fromCallable { Factory.AccountId(accountIdRaw) }
            .flatMap { accountId -> transactionQueryService.findAllByAccountId(accountId) }
            .map { findAllResult -> Serializer.FindAllResponse(findAllResult) }
}